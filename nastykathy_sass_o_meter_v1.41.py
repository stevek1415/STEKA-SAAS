...
def query(payload):
    ...
    if response.status_code == 200:
        ...
        return [[{"label": "UNKNOWN", "score": 0.0}]]  # Handle errors gracefully
...
if st.button("Sassify Me Kathy"):
    ...
    if st.session_state.input_count == 4:
        st.warning("This is getting boring.. Type **'Screw You Kathy'** in the box if you Dare")
        st.stop()  # Stops the app here until the user types 'Screw You Kathy'

    # Check if the user typed 'Screw You Kathy' after the 4th input
    if user_input.strip().lower() == "screw you kathy":
        st.text_input("Enter your phrase below:", value="", key="cleared_box")
        st.warning("**Screw Me?** - No - **SCREW YOU**, and your newly encrypted files. Want the key? Send **.5 BC** to **Don'tPissOffKathy@aol.com**. Add another .5 BC if you don't want your porn history sent to all the people in your contacts.")
